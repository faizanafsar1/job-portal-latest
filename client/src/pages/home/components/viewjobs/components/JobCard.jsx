...
const JobCard = ({ ref, url, onClick, jobTitle, companyName, companyLocation, jobType, salary }) => {
  return (
    <div
      ref={ref}
      onClick={onClick}
    ...
    >
      <div className="flex justify-between items-center">
        <h2 className="text-lg capitalize font-bold text-gray-900">{jobTitle}</h2>
      ...
      <div className="flex flex-wrap gap-4 w-fit mt-4 space-y-4">
        <span className="px-3 py-1 w-fit text-sm bg-gray-100 text-gray-800 rounded-md">{jobType}</span>
        <span className="px-2 py-1 h-fit w-fit text-xs bg-green-50 text-gray-800 rounded-md">{salary}</span>
      </div>
      <Link
        target="_blank"
        to={`${url}`}
